...
(function () {
  ...
  // 主节点 widgets_values（顺序 = schema widget 顺序，共 28 值 = 27 控件 + 种子 control 占 1 位）：
  // 宽高比, 百万像素, 宽度, 高度, 每段时长, 引导帧数, 种子, 种子control, 步数, CFG,
  // 采样器, 调度器, 自动存档, 存档目录, 桥帧门控, 清晰度阈值, 回退上限,
  // 锚定加噪, 审片模式, 自动保存, 重跑起始段, 接缝重摇, 重摇阈值, 重摇上限,
  // 递减锚定, 生成模式, 自动成片, 导演台状态
  // 注意：提交 7435084 已剔除 接缝处理/混合帧数/精修强度/精修窗口/智能切镜/切镜最多丢帧/
  //      全链丢弃预算/自适应精修 共 8 个控件，并新增「自动成片」，本数组顺序必须与之严格一致。
  const H3_WIDGETS = [
    "16:9", 0.5, 864, 480, 5.0, "22",   // 宽高比…引导帧数（0..5）
    0, "fixed", 25, 1.0,                // 种子 + 种子control + 步数 + CFG（6..9）
    "res_multistep", "simple",          // 采样器, 调度器（10..11）
    "关闭", "", "标注", 30.0, 34,        // 自动存档…回退上限（12..16）
    0.0, "关闭", "分段", 0,              // 锚定加噪, 审片模式, 自动保存, 重跑起始段（17..20）
    "自动", 0.06, 1, "关闭",             // 接缝重摇, 重摇阈值, 重摇上限, 递减锚定（21..24）
    "文生视频", "开启", ""               // 生成模式, 自动成片, 导演台状态（25..27）
  ];
  ...
  // 分段视频不再单独打包落盘：主节点「自动保存=分段」已把每段 mp4/缩略图
