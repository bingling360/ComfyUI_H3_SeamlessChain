...
- **自动保存（默认开）**：「自动保存=分段+成片」时无需任何下游接线——每段生成完自动存 `output/h3_projects/<项目名>/seg_NNN.mp4`，链（或逐段审片已确认部分）自动拼成完整成片 `final_时间戳.mp4` 同目录，报告注明路径，跑完直接去 output 看。
...
- **潜空间放大二采（高清重制）**：主链跑完后对任意段做「神经放大 latent → 低强度重采样补回高频细节」（整合 LBH 潜空间放大网络 + JZL 二采范式），高清分段 `upseg_*.mp4` 与全链高清成片 `final_up_*.mp4` 落项目文件夹；音轨沿用原声零回归；参数不进基础链指纹——改参数只重做二采，不动已生成段；支持跟随生成 / 手动勾选两种模式，插入视频段与序章同样可二采。
...
output/h3_projects/
  ...
  <项目名>/
    ...
    upseg_001.mp4 ...             # 二采高清分段（潜空间放大重采样，可选）
    upthumb_001.png, uplast_001.png  # 二采缩略图 / 尾帧锚（下段接缝平滑用）
    final_up_20260820_200000.mp4  # 二采高清成片（全链 upseg 拼接，可选）
    keyframes/                    # 分镜模式关键帧
    seg_001.pt, upseg_001.pt ...  # 续拍 latent（约 5MB/段，前端不显示）
```
...
采样器自带「自动保存=分段+成片」（默认开）：跑链**不需要接任何下游节点**——每段生成完自动把分段 mp4 存到 `output/h3_projects/<项目名>/seg_NNN.mp4`，链尾（或逐段审片已确认的部分）自动拼成完整成片 `final_时间戳.mp4` 同目录，报告里注明路径，跑完直接去 output 目录看片。想接 `H3ChainSaver` 存历史画廊或 `H3SeamDoctor` 做接缝体检照常可接，两不误；分段复制不重编码（秒级），成片走 PyAV（ComfyUI 新视频栈自带），编码限 4 线程且分块搬运内存（不会打满 CPU / 撑爆内存），终端会打印编码开始与耗时。设为「关闭」则恢复纯输出模式。
...
整合 [LBH-123-AI/Comfyui_Minimax_h3_latent_Upscaler](https://github.com/LBH-123-AI/Comfyui_Minimax_h3_latent_Upscaler) 的潜空间放大网络与 [wjluoxiao/ComfyUI-JZL-MiniMax-H3](https://github.com/wjluoxiao/ComfyUI-JZL-MiniMax-H3) 的二次采样范式：读基础链段 latent → **神经放大 H×W**（时间维不变，24 通道 H3 VAE latent，LATENTS_MEAN/STD 归一化）→ **低强度重采样**（官方 `common_ksampler`，denoise<1 补回放大丢失的高频细节）→ 解码落盘 `upseg_NNN.mp4`。二采是**完全独立的后处理通道**：主循环（生成 / 桥接 / 断点 / 重跑）零改动，任何失败降级为报告一行，不丢基础链产物；节点主输出仍是基础链结果（高清产物在项目文件夹，避免高清全帧驻留内存）。
...
2. 开导演台右栏 **「✦ 潜空间放大二采」** 面板选模式：
   - **跟随生成**：每次运行后自动对新增 / 失效段二采——逐段审片时即「生成一段、二采一段」；
   - **手动选择**：在段落卡片勾选任意已完成段（含插入视频 / 序章），点「开始生成」执行；
   - **关闭**：不执行（已产出的 `upseg_*` 不受影响）；
3. 面板参数：放大模型（目录扫描下拉）、网络架构（2D 残差骨干快 / 纯 3D 卷积时序一致性好，须与权重匹配——不匹配时明确报错）、放大倍率 1.0–4.0（latent 偶数对齐 = 像素 32 倍数，目标画布徽章实时显示）、二采强度 0.05–1.0（0.35–0.55 常用）、二采步数 / CFG（H3 常用 1.0）、精度 fp32/fp16/bf16；
4. 全链记录齐且链完成时自动拼接高清成片 `final_up_时间戳.mp4`（流式拼接，复用合并导出的拼接器）；段卡片显示「二采 W×H」徽章，不满意的段点「↺ 重置二采」清记录下次重做。

关键语义：

- **音轨沿用原声**：重采样输出的音频丢弃，成片音轨 = 原轨（零音频回归，符合「拼接不动音频」铁律）；段首仍做 ±6dB 钳制响度对齐，与基础链同口径；
- **重做规则**：二采参数变（指纹变）→ 范围内段全部重做二采，基础链不动；基础链从段 k 重做 → `upseg ≥ k` 记录与文件自动清除，链完成后补做；某段提示词 / 种子变（基础身份指纹变）→ 仅该段二采失效；单段二采只依赖「该段基础 latent + 上段基础 latent + 二采参数」，与处理顺序 / 模式无关，可独立重做；
- **接缝质量**：桥锚走 CondSync（上段基础 latent 尾切片神经放大后注入 keyframe，避免按原尺寸重编码）；上段有二采产物时段首做 smoothstep 接缝平滑（锚定上段高清尾帧）；种子 = 基础段种子 + 1 派生，同输入重放一致；
...
- 「分段图像 / 分段音频」是列表输出：下游 `CreateVideo → SaveVideo` 会**每段各执行一次**，节点预览框因此只显示其中一段（ComfyUI 列表批处理的固有行为，不是丢段）——全部分段视频都按编号存进了输出目录；逐段审看直接去项目文件夹 `output/h3_projects/<项目名>/` 播放 seg_NNN.mp4，或 `H3ChainSaver` 画廊「本次分段」页。
