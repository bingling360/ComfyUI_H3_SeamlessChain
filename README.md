...
- **🧪 实验性功能（生成期 in-process 干预）**：导演台面板底部单列「实验性功能」分区，含多项可单独/组合开启的生成期干预——**E1 强化引导桥+段内分片**（滑窗重叠条件采样、段内拆子片抑制漂移）、**E2 全局记忆锚**（首段关键帧沿链恒定注入）、**E3 运动感知闭环门控**（光流/相机 z-score 扩展重摇触发信号）、**E4 双向过渡重生成**（三窗双锚+缝区独占噪声定向重采样）。全部默认关闭，后端硬开关 `H3_EXPERIMENTS=0` 可强制全关；切换实验组合触发整链重做，保证 A/B 对照干净。详见 [`更新说明_实验性功能.md`](更新说明_实验性功能.md)。
- **🪂 桥区软着陆 / 音频接缝软过渡 / 段中锚点**（ComfyUI v0.34+ 新版桥接能力）：段首直接钉住上一段的真实尾巴（逐 token 噪声掩码），替代"多烧 ctx 帧再裁掉"；另两项分别负责音频接缝的响度残留强度与段中漂移抑制。同样以实验开关形式提供，默认关闭，不满足版本要求自动回退。详见 [`更新说明_软桥着陆与多锚点.md`](更新说明_软桥着陆与多锚点.md)。
...
- 「**桥区软着陆**」（实验性功能）额外需要 ComfyUI **≥ v0.34.0** 的**逐 token 视频/音频噪声掩码**（`denoise_mask` / `audio_denoise_mask`）与官方 `MiniMaxH3AddGuide`（任意帧锚定）。不满足时自动回退现状「钉桥 + 裁头」路径并在报告首部注明，不影响出片。
- 不确定本机支持到哪一步？跑一次自检（AutoDL / 本机均可，只读不写、秒级）：

  ```bash
  cd <你的 ComfyUI 目录>
  python custom_nodes/ComfyUI_H3_SeamlessChain/tools/probe_bridge_caps.py
  ```

  输出里「软桥等级」为 2 = 全能力可用；1 = 保留生效但模型侧无逐行 timestep（建议升级）；0 = 走现状路径。
...
### 桥区软着陆（实验性功能，需 ComfyUI ≥ v0.34.0）

**现状链路（钉桥 + 裁头）**：每段多采样 `引导帧数`（ctx）帧，把上段尾 latent 作为 keyframe 钉在条件里，采样后把这 ctx 帧**整个裁掉**——它们只是"预热"，成片用的是后面重新生成的画面。代价：① 白算 ctx 帧（默认 22/146 ≈ 15%）；② 接缝第一帧仍是模型自由生成的，与条件里的上段尾只有软关联。

**软桥**：把上段尾 latent 直接写进本段初始 latent 的头部，并用**逐 token 的噪声掩码**（`denoise_mask` / `audio_denoise_mask`，走官方 `SetLatentNoiseMask` 的 `latent["noise_mask"]` 入口）把它标记为"已有内容"——采样器每步把钉住区拉回上段尾（等级 1），模型也把它当干净行看待（等级 2）。本段是**接着上段真实结尾画**，而不是照条件重画一遍再裁掉。

- 只烧「钉住帧数」（默认 9 帧 = 3 token）而不是整段 ctx 帧 → 每段少算 ctx−9 帧，接缝反而更紧。
- 音频头部同帧钉住 → 接缝逐帧连续；`音频接缝软过渡` 开关里的「响度对齐强度」默认 0（不再叠加增益），需要时可调回。
- 钉住窗之外一律完全生成（掩码 = 1）。**为什么默认不是"斜坡"**：钉住区之外 `latent_image` 是空的，掩码 <1 会把行拉向全零 latent（黑帧 / 静音）。`linear / smoothstep / ease_in` 三档只作 A/B 对照（让钉住窗尾部提前放开），默认 `hold` = 整窗钉死。
- 钉住帧数会自动落到真实 token 网格（5 / 9 / 13 / 17 / 18 / 22 …），因为 latent 不能切半个 token 且 token↔帧映射不等距。
- 任何不满足条件（钉住帧数不足 1 token / 上段无桥 / 装配异常 / ComfyUI 版本不够）都**回退现状路径**：预检不通过走现状段长，装配异常只丢钉住效果、形状保持一致，报告逐段注明。

开关在导演台「实验性功能」面板：`桥区软着陆`（钉住帧数 / 释放曲线）、`音频接缝软过渡`（响度对齐强度）、`段中锚点`（锚点位置）。全部默认关闭；切换组合触发整链重做，A/B 对照干净。详见 [`更新说明_软桥着陆与多锚点.md`](更新说明_软桥着陆与多锚点.md)。
...
- 「桥区软着陆」开启时，段的**采样帧数**由 `段长 + 引导帧数` 变为 `段长 + 钉住帧数`，因此自动命名的断点目录会换一个（指纹变了）；自定义命名的目录因存档缺新键而可能被复用——所以开关与参数变化一律走**实验指纹双向比对**（关→开、开→关、换参数都判不一致并要求新目录），不会静默混用不同桥模式的段。
- 「桥区软着陆」与「段中锚点」均为**新能力**，尚未经过长链实测标定：先用 3 段短链 + 同一条链开关前后各跑一次（A/B）比对 `接缝平均帧差` 与 `光流 z`，确认收益后再上长片。软桥只作用于**需要采样的提示词段**：插入视频段与上一段之间仍是硬切（设计如此），但插入段**之后**的第一个提示词段照样用软桥从插入段结尾续拍。
