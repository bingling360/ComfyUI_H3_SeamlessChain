...
MiniMax H3 长视频插件：**段间引导续拍** + **分镜模式** 双节点。
...
- **「长片审片」侧栏面板**：ComfyUI 左侧新增面板（旧版前端为悬浮按钮），逐段缩略图 + 种子 + 接缝/桥分一目了然，「继续下一段 / 重摇此段 / 改词重跑」一键完成；数据走 ComfyUI 自带 `/api/view`，零自建路由、零新依赖。
- **序章续拍**：上传一段现成视频（≥5 帧、24fps）接「起始视频」，编码为第 0 段进断点——成片以它开头，生成段从它的结尾无缝续拍。
...
- **接缝测量 + smoothstep 兜底**：每段报告帧差与音频响度跳变（诊断模型起始漂移）；`接缝混合=smoothstep`（默认开）在拼接点做像素级兜底——上一段尾帧硬锁为本段首帧（逐像素一致），smoothstep 权重窗平滑吸收前几帧偏差，音频同步从上段尾音渐入（与一体化总控台 `LtoJ_H3ContinuityOpeningLock` 同机制）。
...
装好插件重启后，ComfyUI 左侧侧栏多一个「长片审片」标签（**播放机图标** 🎬 `pi pi-video`；旧版前端降级为右侧悬浮按钮）。面板数据全部来自断点目录，运行一次工作流后自动出现各段卡片，**每段卡片左侧直接内嵌播放器，点开即看该段带声音的完整视频**（H.264 mp4，由节点自动逐段落盘）。四个常见问题的操作路径：

- **怎么确认这次运行接在上一段后面？** 看面板顶部状态条：「已载入段 1–N（断点回放）→ 本次将生成段 N+1，锚定段 N 尾部」。每张已完成段卡片有可播放的分段视频（或缩略图）、种子、接缝分 / 桥分、提示词摘要，所见即所得。
...
2. 运行：上传视频被编码为**第 0 段（序章）**存入断点（面板上带「序章（上传）」徽标），成片以它开头，第 1 个生成段从它的结尾续拍（面板 / 报告中段号相应 +1）。
3. 约定：按 **24fps** 处理（与官方参考视频一致）；帧数向下对齐到 17k+5 网格，超过「每段帧数」只取前段；序章经一次 VAE 重编码（报告注明），画面会有极轻微的重编码损失。
4. 换视频 = 换链：序章指纹（帧数 + 首帧统计）写入哈希链，更换上传视频后自动整链重做；序章本身不能「重摇」，面板上两个重跑按钮对序章禁用。
...
- 纯像素后处理，不进断点指纹：已完成的链改窗长或开关都不触发重跑，断点回放段输出与一次跑完逐帧一致。
