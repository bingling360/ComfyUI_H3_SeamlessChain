...
- **抢占式显存管理**：H3 UNET fp16 常驻 ~26GB，2× 高清激活约为基础 4 倍。重采样前 `unload_all_models()` 把 CLIP/videoVAE/audioVAE 卸到 CPU（`common_ksampler` 只回载 UNET），放大网络完成后也卸回 CPU；预检判定不足或实际 OOM 时明确停止整链并提示降倍率 / 步数 / σ，避免静默产出高清与基础分辨率混排；
- **接缝质量**：二采在接缝判定前接管——上段有高清产物时高清帧段首做 smoothstep 平滑（锚定上段高清尾帧）；种子 = 基础段种子 + 1 派生，同输入重放一致；
- **失败处理**：放大模型缺失 / 不匹配在开跑前报告并跳过本轮二采；画布 / 显存预检失败和采样 OOM 会停止整链；其他单段异常记录后回退该段基础保存并强制重编码自愈；
