...
- **潜空间放大二采（高清重制）**：每段采样定稿后、解码之前立即「神经放大 latent → 低强度重采样补回高频细节」（整合 LBH 潜空间放大网络 + JZL 二采范式），**分段视频与成片直接保存二采后的高清结果**（同名覆盖，单份产物，无额外解码）；音轨沿用原声零回归；参数不进基础链指纹——改参数只重做二采，不动已生成段；支持跟随生成 / 手动勾选两种模式，插入视频段与序章同样可二采。
...
4. 全链记录齐且尺寸一致时成片**直接流式拼接高清分段** `final_时间戳.mp4`（不再额外产出基础分辨率副本；记录不齐 / 混排 / 拼接失败回退基础编码，分段高清不受影响）；段卡片显示「二采 W×H」徽章，不满意的段点「↺ 重置二采」清记录下次重做。
...
- **接缝质量**：桥锚走 CondSync（本段生成时的桥锚 / 尾锚 latent 同步神经放大后注入 keyframe）；上段有高清产物时段首做 smoothstep 接缝平滑（锚定上段高清尾帧）；种子 = 基础段种子 + 1 派生，同输入重放一致；
- **失败降级**：模型缺失 / 不匹配当场报错整链按基础分辨率运行（报告注明）；单段渲染失败只降级该段（回退基础保存并强制重编码自愈），整链照常；
