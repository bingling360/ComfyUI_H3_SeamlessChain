...
装好插件重启后，ComfyUI 左侧侧栏多一个「长片审片」标签（胶片图标；旧版前端降级为右上角悬浮按钮）。面板数据全部来自断点目录，运行一次工作流后自动出现各段卡片。四个常见问题的操作路径：

- **怎么确认这次运行接在上一段后面？** 看面板顶部状态条：「已载入段 1–N（断点回放）→ 本次将生成段 N+1，锚定段 N 尾部」。每张已完成段卡片有缩略图、种子、接缝分 / 桥分、提示词摘要，所见即所得。
...
面板按钮改的都是画布上 `H3SeamlessChainSampler` 节点的控件再排队，与手动操作完全等价；改词写回支持节点内输入框和上游 `PrimitiveStringMultiline` 两种接法。旧断点目录（无缩略图 / 无状态文件）面板会降级为占位显示，不报错。
