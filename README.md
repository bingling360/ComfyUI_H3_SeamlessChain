...
- `example_workflows/h3_chain_storyboard_review.json`：**长片审片生产工作流（推荐，综合生态各家之长）**——四段分镜提示词节点 + 审片模式 + 断点续拍 + 桥帧门控标注 + 成片/分段双保存支路 + 图内使用说明。流程吸收：Contex-Loop 的分镜单与逐段审片门、Motion-Context 的接缝只测不管、Director 的每段即时落盘视频、总控台的生成前打分门控，底座为本项目的 latent 断点引擎。装好即按说明「运行 → 看分段 → 确认 → 再运行」循环到成片。
