...
- `断点续拍=自动续跑`：每段采样完成即写 `output/checkpoints/<断点目录>/seg_NNN.pt`（AV latent）+ `manifest.json`（原子写）。第 k 段崩溃后**原参数重跑**，前 k 段自动从断点载入重解码（秒级），从第 k+1 段继续采样。
- 断点目录默认按「提示词 + 全部参数」指纹自动命名；`断点目录` 控件可指定固定名。**改任何参数/提示词都会拒绝续跑**（报错提示开新链）；删掉某个 `seg_NNN.pt` 则该段起重采样。
- 种子以 manifest 为准：种子控件开着自动 +1，续跑时报告会注明「控件种子被忽略，沿用断点种子」。
