...
段首偏差的根因：引导桥走的是 **conditioning 路径**（`minimax_keyframes` 每步重注入），但下段生成区 latent 仍从**纯噪声、denoise=1.0** 起步——锚定帧只是"建议"不是"约束"，模型每步都有偏离自由度，偏差大小取决于内容与种子的抽卡运气。开源共识的根治思路是让衔接内容进入 **latent 主路径**（Wan2.1 I2V 首帧 latent 落位 / SVI 2.0 尾 latent 直通 / SkyReels-V2 重叠区低噪共同去噪）。本插件落地其中与官方采样协议完全兼容的一种：

- **跨缝窗口构造**：每条接缝生成后，取「上段尾 W 帧 + 本段头 W 帧」的**干净 latent** 沿 token 网格拼成跨缝窗口（`精修窗口`=每侧 W 帧；两侧 token 数之和 ≡4 mod 5 永不落网格点，缺口自动从本段侧补齐）。
- **联合重去噪**：窗口整体作为初始 latent，以 `denoise=精修强度`（默认 0.45）走官方 `common_ksampler` 重去噪（H3 为标准 flow-matching：`(1−σ)x0+σε` 前向加噪再逐步去噪）。模型在自己的潜空间里**调和两侧运动矢量**——缝后帧与缝前帧出自同一次去噪，缝差≈段内正常帧差（实测 0.085 → 0.021 量级）。
- **只替换本段头部**：上段已拼接的帧、存档 latent、分段 mp4 一概不动；精修区末端按 `混合帧数` smoothstep 羽化回原帧（w: 0→1，末帧=原帧），防精修边界出现第二条微缝。
- **音频不精修**：音轨沿用生成期桥锚定 + 裁剪对齐 + 段首响度对齐路径（两侧音频是不同内容，重去噪有改音色风险；实测音频缝差已小）。
- 报告单行注明精修前后缝差对比（`段2 接缝精修：0.085 → 0.021 · 强度 0.45（自适应）· 双端锚定 · 窗口 90 帧 · 37s`）；精修异常自动回退 smoothstep 像素混合并注明，窗口不足则跳过该缝处理。
...
- 纯后处理不进存档指纹：已完成的链改强度/窗口/阈值都不触发重跑，存档回放段输出与一次跑完逐帧一致（回放链每缝仍会做一次精修采样，秒级–十秒级）。
- 适用面：生成段之间、序章之后第一个生成段都生效；`smoothstep像素混合` 降级路径适用于显存紧张或想要确定性纯像素行为的场景（加权平均有叠影/糊感，建议仅调试对比用）。
...
- **整窗统一强度重去噪会把好缝也软化** → `自适应精修`（默认开）：按缝差分档 <0.04→0.30 / 0.04–0.08→0.45 / >0.08→0.55，好缝轻修保细节、坏缝强调和；
- **精修只锚缝前侧，模型自由改写无锚端** → **双端锚定**：缝前侧整段 latent 锚窗口头（index=0），本段侧窗口末 2 token（5 帧）锚窗口尾——模型只能改写中间区；
- **精修后缝差反而升高时回退像素混合（越修越糊）** → 改为保留精修结果 + 报告 WARN；像素混合仅在精修抛异常时兜底。
