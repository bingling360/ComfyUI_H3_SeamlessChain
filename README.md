...
- **两段式（显存根治）**：H3 UNET fp16 常驻 ~26GB，在高清 latent 上重采样必 OOM。故①低强度重采样在**基础分辨率** latent 上做（显存=基础采样水平）补细节，②放大网络单独超分 latent 到 scale×（1-2GB 网络可跑）提分辨率——不再带高清激活跑主模型；桥锚 / 尾锚直接沿用基础尺寸注入 cond（重采样画布=基础，无需放大桥）；
