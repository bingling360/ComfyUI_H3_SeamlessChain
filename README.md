...
MiniMax H3 长视频插件：**续拍 / 分镜 / 控制台 / 成片保存** 全套长片生产线。
...
- `H3ChainConsole`（控制台）：画布上的「导演台」——存档读档/新建/删除、各段提示词、首帧/序章上传、段卡片审片重摇。
...
- **控制台节点 `H3ChainConsole`**：画布上的「导演台」——存档读档下拉（含进度/日期/提示词预览）、时间戳新建、删除、旧存档提示词一键回填、首帧/序章上传、各段提示词填写；段卡片内嵌播放器 + 种子/接缝/桥分 + 「继续下一段 / 重摇此段 / 改词重跑」一键完成。
- **成片保存节点 `H3ChainSaver`（一体化）**：最终成片 PyAV 编码落盘 + 分段视频从存档复制（不重编码），节点内画廊三分屏——**成片历史 / 本次分段 / 当前成片**，可播放、下载、删除，跑过的片子一目了然。
- **序章续拍**：上传一段现成视频（≥5 帧、24fps）——控制台面板直接上传即得序章（或接 `LoadVideo` 到「起始视频」），编码为第 0 段进存档：成片以它开头，生成段从它的结尾无缝续拍。
...
- **接缝测量 + smoothstep 兜底**：每段报告帧差与音频响度跳变（诊断模型起始漂移）；`接缝混合=smoothstep`（默认开）在拼接点做像素级兜底——上一段尾帧硬锁为本段首帧（逐像素一致），smoothstep 权重窗平滑吸收前几帧偏差（与一体化总控台 `LtoJ_H3ContinuityOpeningLock` 同机制）。
...
- `接缝混合`：默认 `smoothstep`——拼接点像素级兜底，上一段尾帧硬锁为本段首帧 + smoothstep 窗吸收前几帧偏差，音频同步渐入；`关闭` = 拼接点保持硬切（仅测量）。不进断点指纹，改此参数不触发重跑。
- `混合帧数`：smoothstep 窗长，默认 6（≈0.25 秒 @24fps）。运动越快窗应越短；主体位移差大时长窗会拉长叠影。
...
- 存档目录默认按**共享参数**（分辨率 / 段长 / 引导帧 / 采样参数等，不含提示词）指纹自动命名；`存档目录` 控件可指定固定名（接控制台「存档名」输出即可用读档下拉选择）。**改共享参数会拒绝续跑**（报错提示开新链）；改某段提示词不再换链，而是自动从该段重做（见「任意段重跑」）；删掉某个 `seg_NNN.pt` 则该段起重采样。
- 种子以 manifest 记录的**种子序列**为准：种子控件开着自动 +1，续跑 / 审片续接时报告会注明「控件种子仅在重跑起始段 > 0 时生效，当前沿用存档种子序列」——多轮运行与一次跑完逐帧一致。
- 存档为中间产物，全链跑完可整目录删除（控制台「🗑删除存档」或成片保存节点删除历史文件）；想换条新链，控制台「＋新建」（时间戳名）或改「存档目录」名即可。
...
2. 满意 → **直接重新运行**（或控制台点「▶ 继续下一段」），自动从存档载入已完成段（秒级），生成第 2 段……依此到末段。
3. 不满意 → 两个选择：
...
`H3ChainConsole` 是画布上的「导演台」，旧版左侧「长片审片」面板已退役——存档管理、提示词、上传、段卡片审片全部收进节点本体：

- **存档栏**：下拉读档（每项含 `名字 · done/total · 日期 · 首段提示词预览`）、「＋新建」（时间戳名，= 开新链）、「⤺回填提示词」（旧存档的提示词一键填回本节点的提示词输入框）、「🗑删除存档」。「存档名」输出接采样器「存档目录」——选定存档后运行即从该存档续拍，像游戏读档一样。
- **提示词组**：每段一框、按行 join 成「提示词清单」输出接采样器（改词=改自己控件，不用跨节点找输入框）；空值自动剔除，段序按编号。
- **上传**：面板按钮直接上传首帧图片（i2v）或序章视频（≥5 帧、24fps），按类型自动分流到「首帧」或「序章视频+序章音轨」输出，接采样器对应输入即可。
- **段卡片**：运行一次后自动出现各段卡片——内嵌播放器（H.264 mp4 带声音，或缩略图）、种子、接缝分 / 桥分、提示词摘要。「🎲重摇此段」= 自动设「重跑起始段=N」+ 随机新种子并排队，跑完自动复位；「✏改词重跑」= 卡片内直接改这段提示词，「保存并运行」后从该段自动重做（哈希变化自动定位，无需设任何控件）；末尾「▶ 继续下一段」适配逐段审片节奏。
- 控制台按钮改的都是画布上 `H3SeamlessChainSampler` 节点的控件再排队，与手动操作完全等价；旧存档（manifest 缺缩略图 / 分段视频 / 提示词等新键）在**下一次运行时自动补齐**（纯回放也会回写，不重跑采样）；分段视频依赖 PyAV（ComfyUI 新视频栈自带），缺失时卡片回退为缩略图，不影响生成。目录列举与上传走插件自带路由，路由不可用时自动降级（手输存档名 + 当前链浏览）。
...
怀疑"迷之跳变"时，把 `H3SeamDoctor` 接到采样器旁边即可（**只测不治**，不动生成逻辑）：「图像」「音频」「帧率」接采样器对应输出；再接采样器「分段图像」输出到「分段图像」输入可**精确定位每个接缝**并额外校验拼装一致性与帧数守恒（不接则按帧差阈值自动检测跳变点）。运行后右键预览「报告」——每个接缝一段细粒度体检：
...
把报告（重点最差接缝的 [曲线][位移][断层] 三行）反馈给开发即可针对性修复。「对比图」输出是每接缝一行的 缝前帧|缝后帧|残差伪彩(×4) 拼图，接 `PreviewImage` 直观看。
...
1. 接线（二选一）：控制台面板上传视频 →「序章视频 + 序章音轨」输出接采样器对应输入；或 `LoadVideo` 的 `frames` → 「起始视频」、同节点 `audio` 输出 → 「起始视频音轨」（不接则序章按静音处理）。
2. 运行：上传视频被编码为**第 0 段（序章）**存入存档（控制台段卡片带「序章（上传）」徽标），成片以它开头，第 1 个生成段从它的结尾续拍（控制台 / 报告中段号相应 +1）。
3. 约定：按 **24fps** 处理（与官方参考视频一致）；帧数向下对齐到 17k+5 网格，超过「每段帧数」只取前段；序章经一次 VAE 重编码（报告注明），画面会有极轻微的重编码损失。
4. 换视频 = 换链：序章指纹（帧数 + 首帧统计）写入哈希链，更换上传视频后自动整链重做；序章本身不能「重摇」，控制台上两个重跑按钮对序章禁用。
...
生成前的 latent 引导管「画得像」，接缝混合管「拼得上」：生成后、拼接前，把上一段最后可见帧作为锚帧——**硬锁为本段第 0 帧**（逐像素一致，拼接点帧差恒为 0），再用 smoothstep 权重 `w(t)=t²(3−2t)`（两端导数为 0，无可见速度突变）把本段前 `混合帧数` 帧与锚帧混合，平滑吸收模型起始偏差；音频用同一权重从上一段尾音（约 0.25 秒）渐入本段。三点说明：

- 报告里的「接缝：帧差 / 响度跳变」仍是**混合前**的模型偏差（诊断价值：漂移大时优先重摇该段，而不是靠混合硬压）；混合动作单独一行注明。
- 纯像素后处理，不进存档指纹：已完成的链改窗长或开关都不触发重跑，存档回放段输出与一次跑完逐帧一致。
- 适用面：生成段之间、序章之后第一个生成段（锚 = 序章尾帧）都生效；高速运动保持默认 6 帧，位移差大的画面可降到 3 帧或 `关闭`。
...
- `example_workflows/h3_chain_console.json`：**控制台生产工作流（推荐起点）**——`H3ChainConsole`（提示词 + 存档管理 + 上传）→ `H3SeamlessChainSampler`（逐段审片 + 自动存档）→ `H3ChainSaver`（成片 + 分段一体化保存）。三个节点即一条完整生产线：控制台填词选档 → 运行 → 段卡片审片重摇 → 成片画廊收片。
...
- 「分段图像 / 分段音频」是列表输出：下游 `CreateVideo → SaveVideo` 会**每段各执行一次**，节点预览框因此只显示其中一段（ComfyUI 列表批处理的固有行为，不是丢段）——全部分段视频都按编号存进了输出目录；逐段审看用控制台节点段卡片的内嵌播放器，或 `H3ChainSaver` 画廊「本次分段」页。
